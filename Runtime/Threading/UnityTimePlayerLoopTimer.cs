...
{
    /// <summary>
    /// A timer that uses <see cref="Time.time">Time.time</see> to time and handles non-immediate callbacks in a specific player loop phase.
    /// </summary>
    public sealed class UnityTimePlayerLoopTimer : ITimer, IPlayerLoopItem
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="UnityTimePlayerLoopTimer"/> class.
        /// </summary>
        ...
        public bool Change(TimeSpan dueTime, TimeSpan period)
        {
            ...
            lock (_lock)
            {
                if (_disposed)
                {
                    throw new ObjectDisposedException(typeof(UnityTimePlayerLoopTimer).FullName);
                }
                _dueTime = dueTime;
                _period  = period;
                if (_scheduled)
